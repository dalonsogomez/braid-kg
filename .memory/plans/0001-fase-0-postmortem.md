...
| ~17:46 | Usuario se va; pide máquina del tiempo + implementación local | Tag `wf-checkpoint-2026-05-02-1746` (`828824c`). |
...
| ~20:48 | Resultados commit + tag `wf-fase-0-blocked-by-gemini-quota-2026-05-02` | Commit `886ada5`, `4d89a34` |
...
| Componente | Estado |
|---|---|
| Cognee 1.0 | Instalado en `~/.wikiforge/cognee-mcp/cognee-mcp/` (uv venv). |
| Stack `.env` | Gemini 3 Flash + `gemini-embedding-001` + Kuzu + LanceDB. |
| Shim cognee-mcp-stdio.sh | Funcional, lee key desde `~/.config/wikiforge/secrets.env`. |
...
| Tags | `wf-checkpoint-2026-05-02-1746`, `wf-fase-0-blocked-by-gemini-quota-2026-05-02`. |
...
2. **Decidir camino de desbloqueo** (ver `0001-fase-0-bootstrap-results.md` "Próxima acción"):
   - Camino 1 (recomendado): activar billing en GCP `WikiForge` #846938751343 y re-ejecutar `index_phase0.py` + `validate_phase0.py`.
...
3. **Si decisión es camino 1:** la indexación + validación tarda ~5-10 min. Si pasa el gate ≥4/5, tag `wf-fase-0-completed-YYYY-MM-DD`.
4. **Cualquier camino:** abrir Fase 1 (CLI `wikiforge`, wrapper MCP de tres niveles, perfil global).

## Aprendizajes para Fase 1 (no son decisiones, son insumos)

1. **Test de stack canónico antes de fijar AGENTS.md.** Cualquier nuevo bullet en sec. 3 debe pasar un mini-smoke test (provider funciona, modelo existe, backend está soportado por el engine en su versión actual). Esto se traduce a un comando `wikiforge doctor` en Fase 1.
2. **Diagnóstico de red por proveedor en `wikiforge init`.** Verificar conectividad a los endpoints relevantes (Ollama R2, HF Hub, Gemini, Anthropic) y reportar al usuario antes de elegir stack.
3. **Suite `wikiforge eval` con quota awareness.** Antes de ejecutar el cognify completo, estimar requests necesarios y verificar contra cuotas conocidas del provider. Abortar con mensaje claro si la cuota es insuficiente.
