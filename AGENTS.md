...
| `braid wiki build` | (Mes 2+) Genera Markdown desde Cognee y compila Astro Starlight. `--obsidian` (ADR 0020) genera una bóveda Obsidian en `.braid/wiki/obsidian/`, crea una bóveda nueva con `--output <dir>` o exporta a una bóveda existente con `--vault <dir>` escribiendo solo en `Braid/<dataset_id>/`. No indexa, no llama al LLM y no promueve memoria. |
| `braid status --json` | (ADR 0018) Emite estado estable y parseable del proyecto activo, perfil global y DuckLake si está disponible. La salida humana de `braid status` se mantiene por defecto. |
| `braid doctor` | (ADR 0018) Diagnostica instalación, resolución de contexto, `.braid/`, legacy drift, agentes, secretos, GitHub y catálogo sin indexar, sin LLM y sin promover memoria. `--json` emite checks estructurados; `--fix` solo aplica reparaciones locales seguras gestionadas por Braid. |
| `braid patterns` | (ADR 0019) Muestra el playbook operativo read-only: frontera de proyecto, diagnóstico, activación de agentes, aislamiento de fixtures y cambios de retrieval guiados por evals. Soporta `--json`. No indexa, no llama al LLM, no promueve memoria y no repara archivos. |
