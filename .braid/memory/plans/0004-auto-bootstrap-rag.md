...
- Comando nuevo `braid claude-session-start`: lee filesystem, reporta estado en una línea, no llama al LLM, p50 <500 ms.
- Comando nuevo `braid claude-init`: cablea hook `SessionStart` en `<git_root>/.claude/settings.json`, idempotente.
- `braid sync` con incremental real (mtime + `last_index.json`) y `asyncio.wait_for(timeout=120)` en `cognify`.
- Hook `Stop` opt-in (default `false`) en `~/.braid/profile/preferences.json`.
...
### 2. Comando `braid claude-session-start`
**Archivo nuevo:** `src/braid/commands/claude.py` (~120 líneas).
**Registro en CLI:** `src/braid/cli.py` añade subparser `claude-session-start` con flags `--json`, `--scope` (default `auto`).
**Lógica:**
...
3. Si hay repo pero no `.kg/`: emit `[Braid] repo no inicializado · ejecuta 'braid init && braid index'`.
4. Si hay `.kg/` pero no `.kg/last_index.json`: emit `[Braid] repo inicializado pero no indexado · ejecuta 'braid index'`.
5. Si ambos existen: lee `last_index.json`, recorre globs canónicos (mismos que `index.py`), cuenta archivos con `mtime > last_index.timestamp`. Cuenta ADRs en `.memory/decisions/`. Emite `[Braid] memoria al día (N inputs · M ADRs)` o `[Braid] memoria stale (X archivos modificados · ejecuta 'braid sync')`.

**Coste**: solo `Path.glob` + `os.stat`. No imports de cognee. **Target p50 <500 ms**.

### 3. Comando `braid claude-init`
**Mismo archivo** `src/braid/commands/claude.py`.
...
4. Si existe: parsea JSON, hace merge sin pisar otras claves. Añade entrada en `hooks.SessionStart` con `command: "braid claude-session-start"`.
...
- `run(rebuild=False, ...)`:
  ...
  - Si quedan 0: `print("[braid sync] al día — sin cambios desde {iso_ts}"); return 0`.
...
- `cognify(dataset)` envuelta en `asyncio.wait_for(_cognify_inner(dataset), timeout=120)` con catch específico de `TimeoutError` que emite el log `[braid runner] cognify timeout (cleanup hang upstream cognee 1.0.5) — datos íntegros`.

### 5. Dogfooding
- Ejecutar `braid claude-init` en el repo Braid mismo → genera `.claude/settings.json` con el hook (NO se versiona — `.gitignore` actual ignora `.claude/`; cada checkout activa el hook con `braid claude-init`).
- Symlink global: `ln -sf <repo>/.venv/bin/braid ~/.local/bin/braid` (necesario porque el hook se ejecuta sin venv activado y `~/.local/bin/` está en el PATH del usuario).
- Añadir `braid_hooks.auto_sync_on_stop: false` a `~/.braid/profile/preferences.json` (opt-in default off, sec. 9.7).

### 6. Verificación

| # | Escenario | Esperado |
|---|---|---|
| V1 | `time braid claude-session-start` en repo Braid x5 | p50 < 500 ms |
| V2 | Mismo, con `--json` | JSON válido con `status:"ready"` o `"stale"` |
| V3 | `braid claude-session-start` en `~/tmp` (sin repo) | exit 0 silencioso |
| V4 | `braid claude-init` en repo nuevo (`/tmp/x` con `git init`) → cat `.claude/settings.json` | JSON válido con hook SessionStart |
| V5 | Mismo, ejecutar 2 veces | idempotente, no duplica |
| V6 | `braid claude-init --remove` | quita solo el hook añadido |
| V7 | `braid sync` recién después de `index` (sin tocar archivos) | exit 0 inmediato con mensaje "al día" |
| V8 | `touch src/braid/cli.py && braid claude-session-start` | mensaje `stale (1 archivo modificado)` |
...
- Tag: `braid-fase-2-auto-bootstrap-rag`.
...
5. ⏳ Hook real en `.claude/settings.json` del repo Braid — visible en próxima sesión.
...
- **Codex CLI / Cursor no consumen `SessionStart`.** Aceptado: este ADR es Claude Code-first; Codex/Cursor llamarán `braid claude-session-start` desde sus propias instrucciones de inicio cuando el usuario lo configure.

## Fuera de alcance

- Resolución root-cause del cleanup async hang upstream (queda como TODO Fase 2 #2, ahora marcado **mitigado** en plan 0003).
- Reranker (TODO #3).
- Suite `braid eval` (TODO #4).
